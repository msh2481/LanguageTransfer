...
#cite(<papadimitriou2020learning>, form: "prose") pre-trained LSTM @hochreiter1997long on structured but not linguistic data, such as MIDI music, Java code, and two synthetic datasets of parentheses. They observed that adapting such model to Spanish by fine-tuning only its input and output embeddings gives better perplexity than when starting from randomly initialized model. Their findings and the experiment setup established a framework which was used in later works and in this one as well. I also use synthethic languages introduced in this work, they will be described in details in the section about synthethic languages.

#cite(<ri2022pretraining>, form: "prose") improved on these results by replacing LSTM with Transformer and by changing the synthethic languages. Both improvements are used in this work as well. #cite(<papadimitriou2023injecting>, form: "prose") used GPT-2 @radford2019language, and I copy hyperparameter choices for datasets from them. #cite(<chiang2022transferability>, form: "prose") introduce a family of Shuffle languages, which I use to create a new synthethic language. #cite(<artetxe2019cross>, form: "prose") used a similar technique to combine a task-specific corpus in English with a corpus in the target language but not related to the task.

It also works in the opposite direction, #cite(<lu2021pretrained>, form: "prose") get performance on different modalities comparable to training from scratch by fine-tuning only the input and output embeddings of the pre-trained GPT-2. They also try different fine-tuning approaches, adapting layer norm parameters and the last Transformer block in addition to input and output embeddings.

An ablation in different direction, #cite(<sinha2021masked>, form: "prose") find that removing all information about word order from the pre-training phase does not affect the final performance very much, given a fine-tuning phase with the correct word order. 

To demonstrate the importance of pre-training task, #cite(<krishna2021does>, form: "prose") sample the training data from a completely artificial language, which consists of random n-grams, and observe that pre-training objectives that require processing this information somehow, such as copying sentences in the right order, still improve performance of the model on summarization tasks compared to randomly initialized version. 

Finally, from the perspective of optimization process, pre-training helps because it moves model parameters into a flat basin of the loss landscape. #cite(<mehta2021empirical>, form: "prose") confirm it and suggest as the reason why pre-trained models are less susceptible to catastrophic forgetting during fine-tuning.  #cite(<neyshabur2020being>, form: "prose") also observe it and also show that models fine-tuned from the same checkpoint stay in the same basin.

== Internal representations

Despite all these surprising findings, language models learn higher-level information as well, sometimes research discovers in what form this information is stored internally. For example, #cite(<gurnee2023language>, form: "prose") showed that pre-trained language models form quite detailed world models, in terms of both time and space representations. Namely, it is possible to extract information about where and when an event happened by linear projection from layer 50 activations on the corresponding token. #cite(<li2022emergent>, form: "prose") trained a language model to predict Othello moves and found that it represented the state of the board internally. #cite(<jin2023evidence>, form: "prose") demonstrate that language models trained on code have representations of program states and that quality of this representations is highly correlated with their ability to generate correct programs.
